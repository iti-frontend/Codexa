        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              ...
              <input type="file" accept="image/*" onChange={handleImageChange} className="text-gray-200 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:bg-indigo-600 file:text-white hover:file:bg-indigo-700" />
            </div>
          </div>
          ...
          {/* Links */}
          <div><label className="block text-gray-200 mb-2">Profile Image</label>
            {formData.links.map((link, i) => (
              ...
              <div key={i} className="flex gap-2 mb-2">
                ...
                <button type="button" onClick={() => removeLink(i)} className="bg-red-500 px-3 py-2 rounded-md text-white hover:bg-red-600">Remove</button>
              </div>
            ))}
            <button type="button" onClick={addLink} className="text-blue-400 hover:text-blue-500">+ Add Link</button>
          </div>

          <div className="flex gap-4">
            <button type="submit" disabled={saving} className="flex-1 bg-blue-600 py-3 text-white rounded-md hover:bg-blue-700">{saving ? "Saving..." : "Save"}</button>
            <button type="button" onClick={() => router.back()} className="flex-1 bg-gray-500 py-3 text-white rounded-md hover:bg-gray-600">Cancel</button>
          </div>
        </form>
      </div>
    </div>
