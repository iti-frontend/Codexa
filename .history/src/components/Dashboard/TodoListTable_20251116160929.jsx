...
export default function TodoListTable({ onTodoUpdate }) {
  ...
  const handleEditTask = (task) => {
    setEditingTask(task._id); // استخدم _id بدل id
  ...
  const handleCheckboxToggle = async (task) => {
    try {
      await todoService.markTodoDone(task._id, !task.isDone); // استخدم _id و markTodoDone
      fetchTodos();
      onTodoUpdate?.();
    } catch (error) {
      console.error("Error updating todo:", error);
  ...
  const filteredTasks = tasks.filter((task) => {
    const matchDate = filterDate && task.dueDate
      ...
      <Table>
        ...
        <TableBody>
          {filteredTasks.map((task, index) => (
            <TableRow key={task._id || `task-${index}`}> {/* استخدم _id */}
