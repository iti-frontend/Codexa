...
export function PostCard({ post }) {
  ...
  const { toggleLike, posts } = useCommunityStore();
  const { userToken, userInfo } = useAuthStore();

  // تحديث حالة اللايك بناءً على بيانات الـ store
  const localPost = posts.find(p => p._id === post._id) || post;
  const localLikes = localPost.likes || [];
  const userId = userInfo?._id || userInfo?.id;
  const liked = localLikes.some(like => {
    if (!userId) return false;
    if (typeof like === "string") return like === userId;
    if (like && typeof like === "object") return like._id === userId || like.id === userId || like.userId === userId;
    return false;
  });
  ...
  const handleEditPost = async () => {
    ...
    try {
      await toggleLike(post._id, userToken); // استخدام الـ store للتحديث
