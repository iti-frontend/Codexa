...
export function PostCard({ post }) {
  ...
  const isLiked = () => {
    ...
    return localLikes.some((like) => {
      if (typeof like === "string") return like === user.uid;
      if (like && typeof like === "object") return like.userId === user.uid || like._id === user.uid;
      return false;
    });
  };
  ...
  const handleLike = async () => {
    ...
    const newLikes = liked
      ? localLikes.filter((like) => (typeof like === "string" ? like !== userId : like.userId !== userId))
      : [...localLikes, { userId }];
          ...
          <div className="flex items-center justify-between pt-4 border-t border-border">
            <div className="flex items-center space-x-6 text-muted-foreground">
              <Button
                ...
                size="sm"
                onClick={handleLike}
                className={`flex items-center space-x-2 ${liked ? "text-red-500 hover:text-red-600" : "text-muted-foreground hover:text-red-500"}`}
              >
