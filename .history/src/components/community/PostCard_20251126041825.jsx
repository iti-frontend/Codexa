...
export function PostCard({ post }) {
  ...
  // دالة اللايك المحدثة - تعمل مثل فيسبوك
  const isLiked = () => {
    ...
    // البحث في اللايكات إذا كان المستخدم الحالي عامل لايك
    const liked = localLikes.some((like) => {
      if (typeof like === 'string') {
        return like === userId;
      }
      if (like && typeof like === 'object') {
        return like._id === userId || like.id === userId || like.userId === userId;
      }
      return false;
    });
  ...
  const handleLike = async () => {
    ...
    const newLikes = wasLiked
      ...
      : [...localLikes, userId]; // إضافة الـ ID فقط
  ...
  const liked = isLiked(); // استخدام المتغير المحلي
...
export function PostCard({ post, onLike }) {
  ...
  // استخدم post._id بدلاً من post.id
  ...
  const handleLike = () => {
    if (onLike) {
      onLike(post._id); // استخدم post._id بدلاً من post.id
    }
  };
