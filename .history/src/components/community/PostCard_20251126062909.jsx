...
export function PostCard({ post }) {
  ...
  useEffect(() => {
    ...
    // تحديد اذا كان المستخدم عامل لايك من قبل
    if (userInfo) {
...
const handleLike = async () => {
  ...
  setIsLiking(true);
  try {
    const res = await toggleLike(post._id, userToken);
    // تحديث اللايكات من الـ store بعد نجاح الطلب
    setLocalLikes(res.likes || []);
  } catch (error) {
    // استرجاع الحالة السابقة لو حصل خطأ
    setLiked(wasLiked);
