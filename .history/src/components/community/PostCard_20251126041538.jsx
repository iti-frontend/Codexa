...
export function PostCard({ post }) {
  ...
  // دالة اللايك المظبوطة زي الأول
  const isLiked = () => {
    if (!user || !localLikes || !Array.isArray(localLikes)) {
      return false;
    }
  ...
  const getAuthorAvatar = () => {
    if (post.user && typeof post.user === 'object' && post.user.avatar) {
      return post.user.avatar;
    }
    
    if (isPostOwner() && user?.photoURL) {
  ...
  const authorName = getUserName(post.user);
