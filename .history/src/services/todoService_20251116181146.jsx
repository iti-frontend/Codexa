...
import { ENDPOINTS } from "@/Constants/شحه-ثىي";
...
export const toggleTodoDone = async (id, isDone) => {
  console.log("Toggle Request:", { id, isDone });
  
  // Try PATCH first, fallback to PUT if CORS blocks it
  try {
    ...
    console.log("Toggle Todo Response (PATCH):", response.data);
    return response.data;
  } catch (error) {
    // If PATCH fails due to CORS, try PUT on the main endpoint
    ...
    console.log("Toggle Todo Response (PUT):", response.data);
    return response.data;
  }
};
