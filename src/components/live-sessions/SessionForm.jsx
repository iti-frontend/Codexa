...
export default function SessionForm({ initialData, isEditMode = false }) {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (initialData.sessionType === 'private' && initialData.course) {
        // Handle both populated object or ID
        const courseTitle = initialData.course.title || courses.find(c => c._id === initialData.course)?.title;
        if (courseTitle) setSelectedCourse(courseTitle);
      }
    }
  }, [initialData, setValue, courses]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      let response;
      if (isEditMode) {
        response = await liveSessionService.updateSession(initialData._id, sessionData);
      ...
      router.push(`/live-sessions/${isEditMode ? initialData._id : response.session._id}`);
      router.refresh();
    } catch (error) {
